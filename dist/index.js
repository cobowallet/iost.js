(function (global, factory) {
	...
	(global = global || self, factory(global.IOST = {}, global.buffer));
	...
	class Blockchain {
	  ...
	  getBlockByHash(hash, complete) {
	    var api = 'getBlockByHash/' + hash + '/' + complete;
	  ...
	  getBlockByNum(num, complete) {
	    var api = 'getBlockByNumber/' + num + '/' + complete;
	    return this._provider.send('get', api);
	  }
	  /**
	   * 获取某个用户的余额
	  ...
	  getBalance(address) {
	    var tokenSymbol = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "iost";
	    var useLongestChain = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
	    var api = 'getTokenBalance/' + address + '/' + tokenSymbol + '/' + useLongestChain;
	    return this._provider.send('get', api);
	  }
	  /**
	   * 获取某个用户的余额
	  ...
	  getToken721Balance(address, tokenSymbol) {
	    var useLongestChain = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
	    var api = 'getToken721Balance/' + address + '/' + tokenSymbol + '/' + useLongestChain;
	    return this._provider.send('get', api);
	  }
	  /**
	  ...
	  getToken721Metadata(tokenSymbol, tokenID) {
	    var useLongestChain = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
	    var api = 'getToken721Metadata/' + tokenSymbol + '/' + tokenID + '/' + useLongestChain;
	    return this._provider.send('get', api);
	  }
	  /**
	  ...
	  getToken721Owner(tokenSymbol, tokenID) {
	    var useLongestChain = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
	    var api = 'getToken721Owner/' + tokenSymbol + '/' + tokenID + '/' + useLongestChain;
	    return this._provider.send('get', api);
	  }
	  /**
	   * 获取智能合约
	  ...
	  getContract(id) {
	    var useLongestChain = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
	    var api = 'getContract/' + id + '/' + useLongestChain;
	    return this._provider.send('get', api);
	  }
	  /**
	   * 获取智能合约下的某个键值
	  ...
	  getContractStorage(contractID, key) {
	    var field = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";
	    var pending = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
	  ...
	  getContractStorageFields(contractID, key) {
	    ...
	    var api = 'getContractStorageFields';
	    return this._provider.send('post', api, query);
	  }
	  /**
	   * 获取account信息
	  ...
	  getAccountInfo(id, reversible) {
	    var api = 'getAccount/' + id + '/' + (reversible ? 1 : 0);
	    return this._provider.send('get', api);
	  }
	  /**
	   * 获取当前Gas费率
	...
	class Transaction {
	  ...
	  getTxReceiptByHash(hash) {
	    var api = 'getTxReceiptByHash/' + hash;
	    return this._provider.send('get', api);
	  }
	  /**
	   * 通过交易哈希查询交易结果
	  ...
	  getTxReceiptByTxHash(txHash) {
	    var api = 'getTxReceiptByTxHash/' + txHash;
	    return this._provider.send('get', api);
	  }
	...
	class HTTPProvider {
	  ...
	  send(method, url, data) {
	    ...
	    return axios$1(config).then(function (response) {
	...
	var HTTPProvider_1 = HTTPProvider;
	...
	class Codec {
	  ...
	  pushInt64(n) {
	    var l = long_1.fromString(n + "");
	    var bb = Buffer.alloc(8);
	...
	*/
	...
	class Signature$2 {
	  constructor(info, keyPair) {
	    ...
	    if (this.algorithm === algorithm.Ed25519) {
	      this.sig = Buffer.from(naclFast.sign(info, keyPair.seckey)).slice(0, 64);
	      this.pubkey = keyPair.pubkey;
	    } else if (this.algorithm === algorithm.Secp256k1) {
	      var secpKey = secp.keyFromPrivate(keyPair.seckey);
	      this.pubkey = keyPair.pubkey;
	      var sig = secpKey.sign(info);
	      var r = sig.r;
	  ...
	  static fromJSON(json) {
	    ...
	    return Object.assign(new Signature$2(), sig);
	  }

	  verify(info) {
	    if (this.algorithm === algorithm.Ed25519) {
	      return naclFast.sign.detached.verify(info, this.sig, this.pubkey);
	    }

	    if (this.algorithm === algorithm.Secp256k1) {
	      var r = new bn(this.sig.slice(0, 32).toString('hex'), 16);
	      var s = new bn(this.sig.slice(32, 64).toString('hex'), 16);
	      var sig = {
	        r,
	        s
	      };
	...
	class KeyPair$2 {
	  constructor(priKeyBytes) {
	    var algType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : algorithm.Ed25519;
	    this.t = algType;
	    this.seckey = priKeyBytes;

	    if (this.t === algorithm.Ed25519) {
	      var kp = naclFast.sign.keyPair.fromSeed(priKeyBytes.slice(0, 32));
	      ...
	      var secpKey = secp$1.keyFromPrivate(priKeyBytes);
	  ...
	  static newKeyPair() {
	    var algType = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : algorithm.Ed25519;

	    if (algType === algorithm.Ed25519) {
	      var kp = naclFast.sign.keyPair();
	      return new KeyPair$2(Buffer.from(kp.secretKey.buffer), algType);
	    }

	    if (algType === algorithm.Secp256k1) {
	      var secpKey = secp$1.genKeyPair();
	      var priKey = Buffer.from(secpKey.getPrivate("hex"), "hex");
	      return new KeyPair$2(priKey, algType);
	    }
	  ...
	  B58SecKey() {
	    return bs58.encode(this.seckey);
	  }
	  /**
	   * 返回公钥的base58编码字符串
	   * @returns {string}
	  ...
	  B58PubKey() {
	    return bs58.encode(this.pubkey);
	  }

	}

	var key_pair = KeyPair$2;
	...
	class Tx {
	  ...
	  addApprove(token, amount) {
	    ...
	    var fixedAmount = amount.toFixed(Math.max(0, (m[1] || '').length - m[2]));
	  ...
	  _base_hash() {
	    var hash = sha3.SHA3(256);
	    hash.update(this._bytes(0));
	    return hash.digest("binary");
	  }

	  addSign(kp) {
	    var sig = new signature$2(this._base_hash(), kp);
	  ...
	  getSignedTxHash() {
	    ...
	    return bs58.encode(hash.digest("binary"));
	  }
	  ...
	  addPublishSign(publisher, kp) {
	    var approveList = this.getApproveList(); // if (approveList.hasOwnProperty("*")) {
	    //     throw "approve should not contain * token";
	    // }

	    this.publisher = publisher;

	    var info = this._publish_hash();

	    var sig = new signature$2(info, kp);
	  ...
	  _bytes(n) {
	    ...
	    for (var _i = 0; _i < this.actions.length; _i++) {
	      var c2 = new codec();
	      c2.pushString(this.actions[_i].contract);
	      c2.pushString(this.actions[_i].actionName);
	      c2.pushString(this.actions[_i].data);
	      c.pushBytes(c2._buf);
	    }

	    c.pushInt(this.amount_limit.length);

	    for (var _i2 = 0; _i2 < this.amount_limit.length; _i2++) {
	      var _c = new codec();

	      _c.pushString(this.amount_limit[_i2].token);

	      _c.pushString(this.amount_limit[_i2].value + "");
	    ...
	    if (n > 0) {
	      ...
	      for (var _i3 = 0; _i3 < this.signatures.length; _i3++) {
	        c.pushBytes(this.signatures[_i3]._bytes());
	      }
	    }
	    ...
	    if (n > 1) {
	      ...
	      for (var _i4 = 0; _i4 < this.publisher_sigs.length; _i4++) {
	        c.pushBytes(this.publisher_sigs[_i4]._bytes());
	      }
	    }
	...
	class TxHandler {
	  ...
	  static SimpleTx(contract, abi, args, config) {
	    var t = new Tx$1(config.gasRatio, config.gasLimit, config.delay);
	...
	class Callback {
	  ...
	  _start() {
	    ...
	    var id = setInterval(function () {
	      ...
	      }).catch(function (e) {// console.log(i)
	      });
	    }, 1000);
	  }
	...
	class IOST {
	  ...
	  callABI(contract, abi, args) {
	    var t = new Tx$2(this.config.gasRatio, this.config.gasLimit);
	    t.addAction(contract, abi, JSON.stringify(args));
	    return t;
	  }
	  /**
	  ...
	  transfer(token, from, to, amount) {
	    ...
	    var t = this.callABI("token.iost", "transfer", [token, from, to, amount, memo]);
	    t.addDefaultApprover();
	    return t;
	  }
	  /**
	   * 新建账号
	  ...
	  newAccount(name, creator, ownerkey, activekey, initialRAM, initialGasPledge) {
	    if (!this._checkPublicKey(ownerkey) || !this._checkPublicKey(activekey)) throw "error public key";
	    var t = new Tx$2(this.config.gasRatio, this.config.gasLimit);
	  ...
	  signAndSend(tx) {
	    ...
	    var cb = new callback(this.currentRPC.transaction);
	    var hash = "";
	    var self = this;
	    self.currentAccount.signTx(tx);
	    setTimeout(function () {
	      self.currentRPC.transaction.sendTx(tx).then(function (data) {
	        hash = data.hash;
	  ...
	  setRPC(rpc) {
	    ...
	    return _asyncToGenerator(function* () {
	      _this.currentRPC = rpc;
	      var requestStartTime = new Date().getTime() * 1e6;
	      var nodeInfo = yield _this.currentRPC.net.getNodeInfo();
	      var requestEndTime = new Date().getTime() * 1e6;

	      if (requestEndTime - requestStartTime < 30 * 1e9) {
	        _this.serverTimeDiff = nodeInfo.server_time - requestStartTime;
	      }
	    })();
	  }
	...
	var iost_js = createCommonjsModule(function (module) {
	  var {
	    Tx
	  } = structs;
	  module.exports = {
	...
	var iost_js_1 = iost_js.IOST;
	var iost_js_2 = iost_js.RPC;
	var iost_js_3 = iost_js.HTTPProvider;
	var iost_js_4 = iost_js.KeyPair;
	var iost_js_5 = iost_js.Tx;
	var iost_js_6 = iost_js.Algorithm;
	var iost_js_7 = iost_js.Account;
	var iost_js_8 = iost_js.TxHandler;
	var iost_js_9 = iost_js.Bs58;
	var iost_js_10 = iost_js.Signature;

	exports.Account = iost_js_7;
	exports.Algorithm = iost_js_6;
	exports.Bs58 = iost_js_9;
	exports.HTTPProvider = iost_js_3;
	exports.IOST = iost_js_1;
	exports.KeyPair = iost_js_4;
	exports.RPC = iost_js_2;
	exports.Signature = iost_js_10;
	exports.Tx = iost_js_5;
	exports.TxHandler = iost_js_8;
	exports.default = iost_js;
